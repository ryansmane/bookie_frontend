...
const AgencyDetail = props => {
   ...
   useEffect(() => {
      async function getAgents() {
         ...
         let myself = await axios.get(`${url}/user/${localStorage.getItem('id')}`, config)
    ...
    const saveAgent = async (id) => {
        ...
        let res2 = await axios.put(`${url}/user/${localStorage.getItem('id')}`, {username:localUsername, email: myEmail, saved_agents: listToBeAdded}, config)
